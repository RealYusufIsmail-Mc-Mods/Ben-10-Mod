...
public class BenTenServerPlaceRecipe<C extends Container> implements PlaceRecipe<Integer> {
    private static final Logger LOGGER = LogUtils.getLogger();
    protected final StackedContents stackedContents = new StackedContents();
    protected Inventory inventory;
    protected BenTenRecipeBookMenu<C> menu;

    public BenTenServerPlaceRecipe(BenTenRecipeBookMenu<C> p_135431_) {
        this.menu = p_135431_;
    }

    public void recipeClicked(ServerPlayer pPlayer, @Nullable Recipe<C> pRecipe,
            boolean pPlaceAll) {
        if (pRecipe != null && pPlayer.getRecipeBook().contains(pRecipe)) {
            ...
            if (this.testClearGrid() || pPlayer.isCreative()) {
                ...
                if (this.stackedContents.canCraft(pRecipe, (IntList) null)) {
                    ...
                    this.clearGrid(true);
                    pPlayer.connection.send(new ClientboundPlaceGhostRecipePacket(
                            pPlayer.containerMenu.containerId, pRecipe));
                }
        ...
        if (flag) {
            for (int j = 0; j < this.menu.getGridHeight() * this.menu.getGridWidth() + 1; ++j) {
                if (j != this.menu.getResultSlotIndex()) {
                    ...
                    if (!itemstack.isEmpty() && Math.min(i,
                            itemstack.getMaxStackSize()) < itemstack.getCount() + 1) {
                        return;
                    }
                }
            }
        ...
        if (this.stackedContents.canCraft(pRecipe, intlist, j1)) {
            ...
            if (this.stackedContents.canCraft(pRecipe, intlist, k)) {
                ...
                this.placeRecipe(this.menu.getGridWidth(), this.menu.getGridHeight(),
                        this.menu.getResultSlotIndex(), pRecipe, intlist.iterator(), k);
            }
        }

    }

    public void addItemToSlot(Iterator<Integer> pIngredients, int pSlot, int pMaxAmount, int pY,
            int pX) {
                    ...
                    if (k == -1 && list.size() <= i) {
                        for (ItemStack itemstack1 : list) {
                            if (itemstack1.sameItem(itemstack)
                                    ...
                                    && itemstack1.getCount() + itemstack.getCount() <= itemstack1
                                        .getMaxStackSize()) {
