...
public class OmnitrixConfig {
    ...
    static {
        ...
        TRANSFORMATION_DURATION_TICKS = builder.comment(
                        "Duration of alien transformation in ticks (default: 3600 = 3 minutes)")
                .defineInRange("transformationDurationTicks", 3600, 200, 72000);

        COOLDOWN_DURATION_TICKS = builder.comment(
                        "Cooldown duration after reverting in ticks (default: 600 = 30 seconds)")
                .defineInRange("cooldownDurationTicks", 600, 0, 36000);

        REVERT_ON_LOW_HP_THRESHOLD = builder.comment(
                        "Health percentage at which the Omnitrix auto-reverts (default: 0.25 = 25%)")
                .defineInRange("revertOnLowHpThreshold", 0.25, 0.0, 1.0);

        ALLOW_CREATIVE_BYPASS_COOLDOWN = builder.comment("Whether creative mode players bypass the cooldown")
