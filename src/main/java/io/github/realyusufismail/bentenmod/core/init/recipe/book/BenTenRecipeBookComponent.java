...
public class BenTenRecipeBookComponent extends GuiComponent implements PlaceRecipe<Ingredient>,
    ...
    private final List<BenTenRecipeBookTabButton> tabButtons = Lists.newArrayList();
    @Nullable
    private BenTenRecipeBookTabButton selectedTab;
    protected StateSwitchingButton filterButton;
    protected BenTenRecipeBookMenu<?> menu;
    protected Minecraft minecraft;
    @Nullable
    private EditBox searchBox;
    private String lastSearch = "";
    private BenTenClientRecipeBook book;
    private final BenTenRecipeBookPage recipeBookPage = new BenTenRecipeBookPage();
    ...
    public void init(int pWidth, int pHeight, Minecraft pMinecraft, boolean pWidthTooNarrow,
        ...
        this.book = BenTenClientRecipeBook
    ...
    public void initVisuals() {
        ...
        for (BenTenRecipeBookCategories recipebookcategories : this.menu
            .getRecipeBookCategories()) {
            this.tabButtons.add(new BenTenRecipeBookTabButton(recipebookcategories));
        }
        ...
        if (this.selectedTab != null) {
            ...
            }).findFirst().orElse((BenTenRecipeBookTabButton) null);
        }
    ...
    private void updateCollections(boolean p_100383_) {
        List<BenTenRecipeCollection> list = this.book.getCollection(this.selectedTab.getCategory());
        list.forEach((p_100381_) -> {
            p_100381_.canCraft(this.stackedContents, this.menu.getGridWidth(),
                    this.menu.getGridHeight(), this.book);
        });
        List<BenTenRecipeCollection> list1 = Lists.newArrayList(list);
    ...
    private void updateTabs() {
        ...
        for (BenTenRecipeBookTabButton recipebooktabbutton : this.tabButtons) {
            BenTenRecipeBookCategories recipebookcategories = recipebooktabbutton.getCategory();
            if (recipebookcategories != BenTenRecipeBookCategories.OMNITRIX_SEARCH) {
    ...
    public boolean mouseClicked(double pMouseX, double pMouseY, int pButton) {
        if (this.isVisible() && !this.minecraft.player.isSpectator()) {
            if (this.recipeBookPage.mouseClicked(pMouseX, pMouseY, pButton,
                ...
                BenTenRecipeCollection recipecollection =
    ...
    private boolean toggleFiltering() {
        BenTenRecipeBookType recipebooktype = this.menu.getRecipeBookType();
    ...
    protected void sendUpdateSettings() {
        if (this.minecraft.getConnection() != null) {
            BenTenRecipeBookType recipebooktype = this.menu.getRecipeBookType();
            boolean flag = this.book.geCustomBookSettings().isOpen(recipebooktype);
            boolean flag1 = this.book.geCustomBookSettings().isFiltering(recipebooktype);
            this.minecraft.getConnection()
                .send(new BenTenServerboundRecipeBookChangeSettingsPacket(recipebooktype, flag,
                        flag1));
        }
